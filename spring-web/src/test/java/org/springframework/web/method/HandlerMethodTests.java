...
/**
 * Backport of {@code AnnotatedMethodTests} from Spring Framework 6.2.11.
 *
 * @author Sam Brannen
 */
class HandlerMethodTests {
	...
	void shouldFindAnnotationOnMethodInGenericAbstractSuperclass() {
		...
		HandlerMethod handlerMethod = new HandlerMethod(new GenericInterfaceImpl(), processTwo);

		assertThat(handlerMethod.hasMethodAnnotation(Handler.class)).isTrue();
	}
	...
	void shouldFindAnnotationOnMethodInGenericInterface() {
		...
		HandlerMethod handlerMethod = new HandlerMethod(new GenericInterfaceImpl(), processOneAndTwo);
